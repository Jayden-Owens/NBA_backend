...
export async function calculateProjection(data, playerMetaMap, teamStats, ID) {
  const meta = playerMetaMap.get(data.PlayerID) || {};
  const DKSalary = meta.salary || 0;
  const Position = meta.position || 'NaN';

  // Run all aggregations in parallel
  const [home_sum, away_sum, last5_sum, opponent_sum] = await Promise.all([
    Player.aggregate([
  ...
  const paceAdjustedProjection = calculatePaceAdjustedProjection(player, teamStats);
  player.FantasyValue = ((paceAdjustedProjection / DKSalary) * 1000).toFixed(3);
  player.PaceAdjustedProjection = paceAdjustedProjection;
